...
This guide explains how to set up and access the PostgreSQL database used by the Auction House monitoring system.

## Overview

The Auction House system uses a **single PostgreSQL database** with **TimescaleDB extension** for all components:
...
DATABASE_URL=postgresql://postgres:password@localhost:5432/auction_house

# Rindexer database connection (usually the same)
RINDEXER_DATABASE_URL=postgresql://postgres:password@localhost:5432/auction_house

# Optional: Separate read-only connection for analytics
READONLY_DATABASE_URL=postgresql://postgres:password@localhost:5432/auction_house
```
...
- **`auction_parameters`**: Contract parameters and metadata
- **`auction_rounds`**: Individual auction rounds (hypertable)
- **`auction_sales`**: Sales within rounds (hypertable)
- **`tokens`**: Token metadata cache
- **`price_history`**: Price data over time (hypertable)

#### Rindexer Tables (Blockchain Events)

Automatically created by Rindexer:

- **`deployed_new_auction`**: Factory deployment events
- **`auction_round_kicked`**: Round start events
- **`auction_sale`**: Individual sale/take events
- **`auction_token_enabled/disabled`**: Token pair events
- **`updated_starting_price`**: Price update events

### Key Views

- **`active_auction_rounds`**: Currently active rounds with calculated data
- **`recent_auction_sales`**: Recent sales with token metadata

### TimescaleDB Features

- **Hypertables**: Time-series optimization for `auction_rounds`, `auction_sales`, `price_history`
...
SELECT * FROM recent_auction_sales LIMIT 10;
...
-- Top auction houses by activity
SELECT
  auction_house_address,
  COUNT(*) as total_sales,
  MAX(timestamp) as last_sale
FROM auction_sales
GROUP BY auction_house_address
...
SELECT add_retention_policy('auction_sales', INTERVAL '90 days');
...
```bash
# Production example
DATABASE_URL="postgresql://auctions_user:secure_password@db.example.com:5432/auction_house?sslmode=require"
```
...
5. **Set up Rindexer**: Configure blockchain indexing with same database
